          ...
          <div class="col-12">
            <div class="card">
              <div class="d-flex card-header bg-light">
                <h4 class="text-capitalize">POTENTIAL DONORS</h4>
              </div>
...
$(document).ready(()=>{
  ...
  $('body').on('click','.deldonor',(e) =>{
          
        let deldonor = e.currentTarget;

        deldonor = $(deldonor).attr('id');
        $.ajax({
          url:'./asset/process.php',
          method:'POST',
          data: {deldonor:deldonor, delz:'deldonor'},
          success:(data, status)=>{
            $('#deldonor').html(data);
            setTimeout(() => {
              $('#deldonor').html('');
